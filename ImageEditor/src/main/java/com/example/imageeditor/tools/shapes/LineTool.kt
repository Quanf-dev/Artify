...
class LineTool : BaseTool() {
    ...
    private var currentDrawable: LineDrawable? = null
    
    private val paint = Paint().apply {
        color = Color.BLACK
        strokeWidth = 10f // Increased default stroke width
        style = Paint.Style.STROKE
        strokeCap = Paint.Cap.ROUND // Added round cap for smoother lines
        strokeJoin = Paint.Join.ROUND // Added round join for smoother corners
        isAntiAlias = true
    }
    ...
    private fun updateDrawablePath() {
        currentDrawable?.updateLine(startX, startY, currentX, currentY)
    }
